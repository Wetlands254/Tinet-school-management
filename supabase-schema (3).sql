...
-- V3 timetable foundation: explicit teacher-to-class-to-subject teaching loads.
create table if not exists public.teaching_assignments (
  ...
  weekly_lessons integer not null default 4 check (weekly_lessons between 1 and 20),
  active boolean not null default true,
  unique(teacher_id, class_id, subject_id, academic_year, term)
);

-- Timetables must be period-aware so a new term/year never overwrites an old timetable.
alter table public.timetables add column if not exists academic_year integer;
alter table public.timetables add column if not exists term text;
update public.timetables set academic_year=coalesce(academic_year,2026), term=coalesce(term,'Term 3');
alter table public.timetables alter column academic_year set default 2026;
alter table public.timetables alter column term set default 'Term 3';

-- V3 timetable uniqueness is per academic period.
alter table public.timetables drop constraint if exists timetables_class_id_day_of_week_start_time_key;
alter table public.timetables drop constraint if exists timetables_teacher_id_day_of_week_start_time_key;
create unique index if not exists timetables_class_period_slot_key on public.timetables(class_id,academic_year,term,day_of_week,start_time);
create unique index if not exists timetables_teacher_period_slot_key on public.timetables(teacher_id,academic_year,term,day_of_week,start_time) where teacher_id is not null;

alter table public.teaching_assignments enable row level security;
-- Keep policies restrictive until an authenticated school-role model is installed.
-- Production deployment should add role-specific SELECT/INSERT/UPDATE/DELETE policies.
